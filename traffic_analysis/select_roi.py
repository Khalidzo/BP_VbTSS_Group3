...
class ROISelector:
    ...
    def select_rois(self, frame):
        ...
        display_frame = frame.copy()  # Initial frame for displaying ROIs
        ...
        while True:
            temp_display_frame = display_frame.copy()  # Create a fresh copy for drawing in each iteration

            # Draw already saved polygons
            for poly_np in self.polygon_np_arrays:  # Use cached numpy arrays for drawing
            ...
            if len(self.current_polygon) >= 3:  # Once 3 points, also draw as closed
                cv2.polylines(temp_display_frame, [np.array(self.current_polygon, np.int32)], isClosed=True,
                              color=(0, 255, 0), thickness=1)

            # Draw red circles for points of the current polygon
            for pt in self.current_polygon:
                cv2.circle(temp_display_frame, pt, 3, (0, 0, 255), -1)  # Slightly larger circles for visibility
            ...
            if key == 13:  # ENTER key
                if len(self.current_polygon) >= 3:
                    ...
                    self.polygon_np_arrays.append(np.array(self.current_polygon, np.int32))  # Cache NumPy array
